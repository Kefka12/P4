...
def writeTunnelRules(p4info_helper, ingress_sw, egress_sw, tunnel_id,
    ...
    table_entry = p4info_helper.buildTableEntry(
        ...
        #由于是入口转发规则，因此动作调用forward
        action_params={
            "port": SWITCH_TO_SWITCH_PORT #换到切换端口，与出口转发骨子额有泣别
        })
    ingress_sw.WriteTableEntry(table_entry)
    print("Installed transit tunnel rule on %s" % ingress_sw.name) 
    #由于是入口转发规则，因此调用ingress_sw
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        # Create a switch connection object for s1 and s2 and s3;
        ...
        while True:
            ...
            printCounter(p4info_helper, s3, "MyIngress.igressTunnelCounter", 100)
            printCounter(p4info_helper, s2, "MyIngress.ingressTunnelCounter", 200)
            printCounter(p4info_helper, s1, "MyIngress.egressTunnelCounter", 200)
            printCounter(p4info_helper, s3, "MyIngress.egressTunnelCounter", 100)
